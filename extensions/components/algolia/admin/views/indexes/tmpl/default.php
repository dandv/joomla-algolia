	...
	<?php endif; ?>
		...
		<?php else : ?>
			<table class="table table-striped" id="articleList">
				...
				<tbody>
					<?php foreach ($indexes as $i => $item) : ?>
						<?php
							$canCreate  = $user->authorise('core.create', 'com_algolia');
							$canEdit    = $user->authorise('core.edit', 'com_algolia.index.' . $item->id);
							$canCheckin = $user->authorise('core.manage',     'com_checkin') || $item->checked_out == $userId || $item->checked_out == 0;
							$canEditOwn = $user->authorise('core.edit.own',   'com_algolia.index.' . $item->id) && $item->created_by == $userId;
							$canChange  = $user->authorise('core.edit.state', 'com_algolia.index.' . $item->id) && $canCheckin;
						?>
						<tr class="row<?php echo $i % 2; ?>">
							...
							<td class="center">
								<div class="btn-group">
									<?php echo JHtml::_('jgrid.published', $item->state, $i, 'indexes.', $canChange, 'cb'); ?>
									<?php // Create dropdown items and render the dropdown list.
									if ($canChange)
									{
										JHtml::_('actionsdropdown.addCustomItem', Text::_('COM_ALGOLIA_BTN_DELETE'), 'trash', 'cb' . $i, 'indexes.delete');
							...
							<td class="has-context">
								<div class="pull-left break-word">
									<?php if ($item->checked_out) : ?>
										<?php echo JHtml::_('jgrid.checkedout', $i, $item->modified_by, $item->checked_out_time, 'indexes.', $canCheckin); ?>
									<?php endif; ?>
									<?php if ($canEdit || $canEditOwn) : ?>
										<a class="hasTooltip" href="<?php echo JRoute::_('index.php?option=com_algolia&task=index.edit&id=' . $item->id); ?>" title="<?php echo JText::_('JACTION_EDIT'); ?>">
