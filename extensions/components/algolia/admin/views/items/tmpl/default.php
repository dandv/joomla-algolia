				...
				<thead>
					<tr>
						...
						<th width="1%" class="nowrap">
							<?php echo JHtml::_('searchtools.sort', 'COM_ALGOLIA_COL_INDEXER', 'i.indexer_name', $listDirn, $listOrder); ?>
						</th>
						<th width="1%" class="nowrap">
				...
				<tbody>
					<?php foreach ($indexers as $i => $item) : ?>
						<?php
							...
							$canEdit    = $user->authorise('core.edit', 'com_algolia.item.' . $item->indexer_id);
							$canCheckin = $user->authorise('core.manage',     'com_checkin') || $item->checked_out == $userId || $item->checked_out == 0;
							$canEditOwn = $user->authorise('core.edit.own',   'com_algolia.item.' . $item->indexer_id) && $item->created_by == $userId;
							$canChange  = $user->authorise('core.edit.state', 'com_algolia.item.' . $item->indexer_id) && $canCheckin;
						?>
						<tr class="row<?php echo $i % 2; ?>">
							...
							<td class="nowrap">
								<?php echo $this->escape(Text::_($item->indexer_name)); ?>
							</td>
							<td class="nowrap">
